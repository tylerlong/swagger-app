...
const reducer = (state, action) => {
  const activeIndex = state.metadata.activePathParameterIndex
  switch (action.type) {
    ...
    case 'DELETE_PATH_PARAMETER':
      return R.pipe(
        R.over(R.lensPath(['pathParameters']), R.remove(activeIndex, 1)),
        R.set(R.lensPath(['metadata', 'activePathParameterIndex']), -1),
        alert('success', 'Path parameter deleted')
      )
    case 'MOVE_PATH_PARAMETER_UP':
      return R.pipe(
        R.over(R.lensPath(['pathParameters']), swap(activeIndex, activeIndex - 1)),
